...
export default async function handler(
  ...
  try {
    ...
    const input = actionInput<SaveInput>(req.body);
    const session = sessionFromRequest(req);
    const members = await graphql<{ org_members: Membership[] }>(
      `
        query SaveMember($orgId: uuid!, $userId: uuid!) {
          org_members(
      ...
      `,
      { orgId: input.organization_id, userId: session.userId },
      { "x-hasura-admin-secret": process.env.NHOST_ADMIN_SECRET ?? "" },
    );
    const member = session.isAdmin
      ? {
          orgId: input.organization_id,
          userId: session.userId ?? "admin",
          role: "owner" as const,
        }
      : members.org_members[0];
    requireRole(member, ["owner", "editor"]);
    const workflow: WorkflowDefinition = {
      id: input.workflow_id ?? "draft",
      orgId: input.organization_id,
      name: input.name,
    ...
    assertSensitiveConfiguration(workflow, member.role);
    const headers = {
      "x-hasura-admin-secret": process.env.NHOST_ADMIN_SECRET ?? "",
    };
    if (input.workflow_id) {
      const existing = await graphql<{
        workflows_by_pk: { org_id: string; description: string } | null;
      }>(
        `
      ...
      if (
        !existing.workflows_by_pk ||
        existing.workflows_by_pk.org_id !== input.organization_id
      ) {
      ...
      await graphql(
        ...
        `,
        {
          ...
          name: input.name,
          description:
            input.description ?? existing.workflows_by_pk.description,
          active: input.active ?? false,
        },
        headers,
      );
      ...
      await graphql(
        `
          mutation ClearChildren($workflowId: uuid!) {
      ...
      const created = await graphql<{ insert_workflows_one: { id: string } }>(
        `
          mutation Save($object: workflows_insert_input!) {
