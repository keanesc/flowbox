...
const normalizeStep = (step: {
  ...
  workflow_id: string;
  position: number;
  type: WorkflowDefinition["steps"][number]["type"];
...
const normalizeTrigger = (trigger: {
  id: string;
  workflow_id: string;
  type: WorkflowDefinition["triggers"][number]["type"];
  config: Record<string, unknown>;
  enabled: boolean;
}) => ({
  id: trigger.id,
...
async function loadWorkflow(workflowId: string): Promise<WorkflowDefinition> {
  ...
  }>(workflowQuery, { id: workflowId }, adminHeaders());
  if (!data.workflows_by_pk) throw new Error("WORKFLOW_NOT_FOUND");
  return {
    id: data.workflows_by_pk.id,
    orgId: data.workflows_by_pk.org_id,
    name: data.workflows_by_pk.name,
    active: data.workflows_by_pk.active,
    steps: (data.workflows_by_pk.workflow_steps ?? []).map(normalizeStep),
    triggers: (data.workflows_by_pk.workflow_triggers ?? []).map(
      normalizeTrigger,
    ),
  };
}
...
async function roleFor(orgId: string, session: Session) {
  ...
  const data = await graphql<{ org_members: Membership[] }>(
    `
      query Member($orgId: uuid!, $userId: uuid!) {
        org_members(
...
class Store implements ExecutorStore {
  async setStepStatus(
    ...
    await graphql(
      `
        mutation Step($id: uuid!, $set: step_runs_set_input!) {
  ...
  async setRunStatus(
    ...
    await graphql(
      `
        mutation Run($id: uuid!, $set: workflow_runs_set_input!) {
    ...
    await graphql(
      `
        mutation Notify($input: notification_deliveries_insert_input!) {
          insert_notification_deliveries_one(object: $input) {
            id
          }
        }
      `,
        ...
        input: {
          ...
          payload: {
            message: String(
              config?.message ??
                config?.title ??
                "Relay Room workflow notification",
            ),
          },
        },
      },
  ...
  async writeResult(input: Record<string, unknown>) {
    const data = await graphql<{ insert_workflow_results_one: { id: string } }>(
      `
        mutation Result($input: workflow_results_insert_input!) {
          insert_workflow_results_one(object: $input) {
            id
          }
        }
      `,
      {
        input: { org_id: input.orgId, run_id: input.runId, value: input.value },
      },
      adminHeaders(),
    );
...
const providers: Providers = {
  async llm(prompt) {
    if (process.env.WORKFLOW_LLM_STUB === "true" || !process.env.GROQ_API_KEY) {
      ...
      return {
        text: prompt.toLowerCase().includes("approve")
          ? "approve"
          : "demo response",
        provider: "stub",
      };
  ...
  async http(config) {
    ...
    const timeout = setTimeout(
      () => controller.abort(),
      Math.min(Number(config.timeoutMs ?? 8000), 15000),
    );
    try {
...
export async function startRun(
  ...
  if (!external)
    requireRole(
      role
        ? { orgId: workflow.orgId, userId: session.userId ?? "", role }
        : undefined,
      ["owner", "editor"],
    );
  ...
  const reserved = await graphql<{ reserve_org_quota: Array<{ id: string }> }>(
    `
  ...
  const run = await graphql<{ insert_workflow_runs_one: { id: string } }>(
    `
      mutation CreateRun($input: workflow_runs_insert_input!) {
        insert_workflow_runs_one(object: $input) {
          id
        }
      }
    `,
  ...
  await graphql(
    `
      mutation Steps($steps: [step_runs_insert_input!]!) {
        insert_step_runs(objects: $steps) {
          affected_rows
        }
      }
    `,
...
export async function approveRun(
  ...
  if (decision !== "approve")
    throw new Error(
      "Only approve is supported; reject is a failed terminal decision.",
    );
  const data = await graphql<{
    step_runs_by_pk: {
      id: string;
      status: string;
      workflow_step: { type: string; position: number };
      workflow_run: {
        id: string;
        status: string;
        workflow: {
  ...
  if (
    !row ||
    row.workflow_run.id !== workflowRunId ||
    row.status !== "paused" ||
    row.workflow_run.status !== "paused" ||
    row.workflow_step.type !== "approval_gate"
  )
    throw new Error("STALE_OR_INVALID_APPROVAL");
  ...
  const update = await graphql<{ update_step_runs: { affected_rows: number } }>(
    `
      mutation Approve($id: uuid!, $userId: uuid!, $approvedAt: timestamptz!) {
        update_step_runs(
    ...
    `,
    { id: stepRunId, userId: session.userId, approvedAt },
    adminHeaders(),
  );
  if (update.update_step_runs.affected_rows !== 1)
    throw new Error("APPROVAL_ALREADY_HANDLED");
  await graphql(
    `
  ...
  const steps = row.workflow_run.workflow.workflow_steps
    .filter((step) => step.position > row.workflow_step.position)
    .map(normalizeStep);
  ...
  try {
    for (const step of steps) {
      ...
      if (step.type === "approval_gate")
        return { runId: workflowRunId, status: "paused" };
    }
    await store.setRunStatus(workflowRunId, "completed", {
      completed_at: new Date().toISOString(),
    });
    return { runId: workflowRunId, status: "completed" };
  } catch (error) {
    await store.setRunStatus(workflowRunId, "failed", {
      error: {
        message: error instanceof Error ? error.message : "Workflow failed",
      },
      completed_at: new Date().toISOString(),
