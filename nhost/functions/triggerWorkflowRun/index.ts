...
export default async function handler(req: { body: unknown; headers: Record<string, string | string[] | undefined> }, res: { status: (code: number) => { json: (data: unknown) => void } }) {
  try { const input = actionInput<{ workflow_id: string; trigger_context?: unknown }>(req.body); json(res, 200, await startRun(input.workflow_id, "manual", input.trigger_context ?? {}, sessionFromRequest(req))); }
