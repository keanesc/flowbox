...
export default async function handler(req: { body: unknown; headers: Record<string, string | string[] | undefined> }, res: { status: (code: number) => { json: (data: unknown) => void } }) {
  try { requireTrustedAction(req); const input = actionInput<{ workflow_run_id: string; step_run_id: string; decision: "approve" | "reject" }>(req.body); json(res, 200, await approveRun(input.workflow_run_id, input.step_run_id, input.decision, sessionFromRequest(req))); }
