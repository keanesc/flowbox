...
### Current acceptance preflight (2026-08-09)

`nhost config validate`, TypeScript, all eight unit tests, and the Next.js production build pass locally. Read-only HTTPS probes returned HTTP 200 from both the hosted frontend and the configured GraphQL endpoint. Nhost's read-only secret listing confirms the deployed `GROQ_API_KEY`, `WEBHOOK_SIGNING_SECRET`, and `RELAY_ACTION_SECRET` names are present (values were not read), and its latest listed deployment succeeded. A clean browser check of the hosted sign-in screen exposed a React hydration mismatch; the source now waits for client hydration before resolving Nhost Auth and has a favicon and credential autocomplete attributes. The repaired build has zero browser-console errors locally, but must be deployed before recording.

No saved Org A owner/editor/viewer or Org B browser session is available in this workspace, OBS is not running, and the local function environment does not contain `RELAY_ACTION_SECRET`. Consequently, no authenticated production mutations, Action calls, subscriptions, webhook/event runs, quota changes, recording, or cross-organization checks were performed in this preflight. Supply existing protected sessions (or authenticate interactively) and start/configure OBS WebSocket before continuing; do not provision users, memberships, or seed data for this purpose.
