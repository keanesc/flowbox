...
import {
  ...
  membershipFor,
  requireRole,
} from "../nhost/functions/_shared/auth";
...
} from "../nhost/functions/_shared/signature";
import { executeStep } from "../nhost/functions/_shared/executor";
import { validateWorkflow } from "../nhost/functions/_shared/validation";

describe("organization and role isolation", () => {
  it("requires both the caller role and the target organization", () => {
    const members = [
      { orgId: "a", userId: "owner-a", role: "owner" as const },
      { orgId: "b", userId: "owner-b", role: "owner" as const },
    ];
    expect(membershipFor(members, "owner-a", "b")).toBeUndefined();
    expect(() =>
      requireRole(membershipFor(members, "owner-a", "b"), ["owner"]),
...
describe("execution safety", () => {
  it("reserves quota atomically at the boundary", () => {
    ...
    expect(() => reserveQuota({ used: 3, limit: 3 })).toThrow(
      "QUOTA_EXHAUSTED",
    );
  });
  it("evaluates only constrained branch expressions", () => {
    expect(evaluateBranch("contains approve", { text: "APPROVE this" })).toBe(
      "true",
    );
    expect(evaluateBranch("equals yes", "no")).toBe("false");
    expect(() => evaluateBranch("value.foo", "anything")).toThrow(
...
describe("external and approval boundaries", () => {
  it("uses timing-safe webhook signatures and rejects tampering", () => {
    const body = webhookSigningPayload("northstar-signal", {
      text: "approve this signal",
    });
    const signature = signWebhook(body, "secret");
    expect(verifyWebhookSignature(body, signature, "secret")).toBe(true);
    expect(
      verifyWebhookSignature(
        webhookSigningPayload("northstar-signal", { text: "tampered" }),
        signature,
        "secret",
      ),
    ).toBe(false);
  ...
  it("pauses at an approval gate", async () => {
    ...
    const result = await executeStep(
      ...
      store,
      { llm: vi.fn(), http: vi.fn() },
    );
    expect(result.output).toEqual({ paused: true });
    expect(store.setRunStatus).toHaveBeenCalledWith("r", "paused");
  });
  it("queues notifications for the Event Trigger instead of claiming synchronous delivery", async () => {
    const store = {
      setStepStatus: vi.fn(async () => undefined),
      setRunStatus: vi.fn(async () => undefined),
      createNotification: vi.fn(async () => undefined),
    ...
    const result = await executeStep(
      {
        ...
        step: {
          ...
          position: 1,
          type: "notify",
          config: { destination: "slack_default", message: "Review required" },
        },
      },
      store,
      { llm: vi.fn(), http: vi.fn() },
    );
    expect(store.createNotification).toHaveBeenCalledOnce();
    expect(result.output).toEqual({
      queued: true,
      destination: "slack_default",
    });
  });
  it("limits notification destinations to the server-configured Slack alias", () => {
    const workflow = {
      id: "w",
      orgId: "o",
      name: "Notify",
      ...
      steps: [
        {
          ...
          config: { destination: "https://attacker.example" },
        },
      ],
      triggers: [],
    };
