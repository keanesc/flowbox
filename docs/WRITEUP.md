...
The checked-in Nhost configuration validates, TypeScript and all eight unit tests pass, and the frontend production build succeeds. Read-only probes reached both the configured GraphQL endpoint and hosted frontend with HTTP 200. Nhost's read-only listing confirms the deployed names `GROQ_API_KEY`, `WEBHOOK_SIGNING_SECRET`, and `RELAY_ACTION_SECRET` are present without exposing their values. A public, clean-browser visit to the hosted sign-in page revealed a React hydration error caused by resolving Nhost auth state differently on the server and initial client render. The source now holds the deterministic connecting shell until hydration; the corresponding Nhost deployment succeeded, and a repeat hosted-browser check has no console errors with all application assets returning HTTP 200.
