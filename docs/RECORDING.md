...
As of 2026-08-09, the local configuration/build checks and read-only endpoint probes pass, and the hydration repair has been deployed and verified against the hosted sign-in page. No protected Org A/Org B session or active OBS instance is available in this workspace. This readiness gate is not passed, so do not start capture or claim any live scenario until the authenticated checks below complete.
